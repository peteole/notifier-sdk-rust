...
pub async fn handle_get_telegram_chat_id(configuration: &configuration::Configuration, username: &str) -> Result<String, Error<HandleGetTelegramChatIdError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/get_telegram_chat_id/{username}", local_var_configuration.base_path, username=crate::apis::urlencode(username));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
